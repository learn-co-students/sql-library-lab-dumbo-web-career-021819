...
def select_character_names_and_number_of_books_they_are_in
  "select characters.name, count(characters.id) from characters join character_books on characters.id = character_books.character_id join books on books.id = character_books.book_id group by characters.name order by count(characters.id) desc;"
end
